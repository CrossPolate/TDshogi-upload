 ...
 *
 * ⚠️ **「香落ち下手先」：本项目决定不采用**（2026-09-23 用户确认：「必须上手先」）。
 *
 * 背景：将棋传统里有「香落ち是例外、由**下手**先手」一说（通称"香落ち下手先"）；
 * 但日本将棋连盟的入门页写的是"必ず上手から"、**未收录该例外**。
 * 本项目一律按 **上手先手**（含香落ち）实现 —— 这也是 `createRoom` 把房主固定为 `b` 的依据。
 * 若将来要改成例外，只需把对应条目的 `firstMover` 改成 `'w'`（局面其余部分不受影响）。
 * ⚠️ 这一条**已经拍板**，别再当成"待确认的约定"重新讨论。
 */
